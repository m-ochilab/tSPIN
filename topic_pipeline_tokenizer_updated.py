...
# ---------------------------------------------------------
# Tokenizer (短文SNS + 日英混在/多言語を想定)
# ---------------------------------------------------------
# CountVectorizer の tokenizer に渡す関数を、research_matae.py の create_tokenizer_latest() で構築。
# 追加で、SNSで重要になりやすいハッシュタグは「#AI -> AI」として残し、@メンションは除去します。
try:
    from research_matae_tokenizer_updated import create_tokenizer_latest  # type: ignore

    _BASE_TOKENIZER = create_tokenizer_latest(
        lang="auto",
        use_lemma=True,
        min_len=1,
        max_len=64,
...
def sns_tokenizer(text: str) -> List[str]:
    """CountVectorizer 用: SNSテキストを token list にする。"""
    if not isinstance(text, str) or not text:
        return []
    t = _URL_RE.sub(" ", text)
    t = _HASHTAG_RE.sub(r" \1 ", t)   # #AI -> AI
    t = _MENTION_RE.sub(" ", t)        # @user は落とす（話題抽出のノイズになりやすい）
    if _BASE_TOKENIZER is not None:
        return _BASE_TOKENIZER(t)
    # 最終フォールバック（最低限動く）
    return t.split()
