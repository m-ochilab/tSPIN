...
# ユーザ名と表示名取得（並列取得）(matae)
    ...
    post_cluster_df = cluster_post_texts(es, query_str, start_date_str, end_date_str, n_clusters=5)
...
def get_block_user_ranking(es, start, end):
    ...
    res = es.search(index="blockindex-*", body=body, request_timeout=es_request_timeout, timeout=es_timeout)
    buckets = res["aggregations"]["users"]["buckets"]
...
def cluster_profile_descriptions(es, n_clusters=5):
    ...
    res = es.search(index="profileindex-*", body=body, request_timeout=es_request_timeout, timeout=es_timeout)
