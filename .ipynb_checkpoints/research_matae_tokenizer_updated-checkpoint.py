...
def tokenize_simple(text: str) -> List[str]:
    """
    短文SNS向けの簡易トークナイザ（後方互換のため関数名は維持）。

    - 可能なら create_tokenizer_latest(lang="auto") を 1 回だけ初期化して再利用
      （spaCy→GiNZA→Sudachi→正規表現フォールバック）
    - それが無理なら従来の正規表現トークナイズにフォールバック

    目的:
      - 日本語/英語混在でも破綻しにくい
      - URL/メンション/ハッシュタグ由来ノイズを抑える
    """
    # lazy init（import 時にモデルを読みに行くと重いので、最初の呼び出しで作る）
    global _TOKENIZE_SIMPLE_CACHE
    try:
        _ = _TOKENIZE_SIMPLE_CACHE  # type: ignore
    except Exception:
        _TOKENIZE_SIMPLE_CACHE = None  # type: ignore

    if _TOKENIZE_SIMPLE_CACHE is None:
        try:
            _TOKENIZE_SIMPLE_CACHE = create_tokenizer_latest(
                lang="auto",
                use_lemma=True,
                min_len=1,
                max_len=64,
    ...
    if _TOKENIZE_SIMPLE_CACHE is not None:
        return _TOKENIZE_SIMPLE_CACHE(text)
